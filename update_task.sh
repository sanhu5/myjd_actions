...
## task清单顶部内容
CommentsLoon="# IOS Loon Task&Cookies配置 By LXK9301\n# GIT_HUB主页(https://GIT_HUB.com/LXK9301/jd_scripts)\n# TG讨论组 (https://t.me/JD_fruit_pet)\n# TG通知频道 (https://t.me/jdfruit)\n# Loon的Task&Cookies脚本订阅链接: https://raw.GIT_HUBusercontent.com/LXK9301/jd_scripts/master/Loon/lxk0301_LoonTask.conf\n# 使用方法:打开APP，顶部的配置 -> 脚本 -> 订阅脚本- > 点击右上角+号 -> 添加url链接 (https://raw.GIT_HUBusercontent.com/LXK9301/jd_scripts/master/Loon/lxk0301_LoonTask.conf)\n\nhostname = api.m.jd.com, wq.jd.com, draw.jdfcloud.com, jdjoy.jd.com, account.huami.com"
CommentsQx='{\n  "name": "LXK9301 task gallery",\n  "description": "https://GIT_HUB.com/LXK9301/jd_scripts",\n  "task": ['
CommentsQxRe="hostname = api.m.jd.com, draw.jdfcloud.com, jdjoy.jd.com, account.huami.com, wq.jd.com"

CommentsSurgeHead="#!name=LXK9301 iOS Tasks&Cookies Module\n#!desc=iOS Tasks&Cookies 模块配置\n\n# Task&Cookies模块配置 By LXK9301\n# GIT_HUB主页(https://GIT_HUB.com/LXK9301/jd_scripts)\n# TG讨论组 (https://t.me/JD_fruit_pet)\n# TG通知频道 (https://t.me/jdfruit)\n# Surge的Task&Cookies脚本模块地址: https://raw.GIT_HUBusercontent.com/LXK9301/jd_scripts/master/Surge/lxk0301_Task.sgmodule.sgmodule\n\n[Script]"
...
do
  ...
  if [[ -n $TaskName ]]; then
    echo -e "\n# $TaskName" >> $FileLoon
    grep -E "cron.+script-path.+https://raw\.GIT_HUBusercontent\.com.+tag" $file >> $FileLoon
    grep -E "https://raw\.GIT_HUBusercontent\.com.+tag.+enabled" $file | perl -pe 's|(.+\w)|\1",|' | perl -pe 's|^|    \"|' >> $FileQx
    grep -E "type.+cronexp.+script-path.+https://raw\.GIT_HUBusercontent\.com" $file >> $FileSurge
  fi
  grep -E "http-(request|response).+script-path.+https://raw\.GIT_HUBusercontent\.com.+tag" $file | perl -pe "s|(.+tag=)(.+)|\n# \2\n\1\2|" >> $FileLoon
  grep -E "script-(request|response)-.+https://raw\.GIT_HUBusercontent\.com" $file | perl -pe "s|(.+)|\n# $TaskName\n\1|" >> $FileQxRe
  grep -E "type=http-(request|response).+pattern.+script-path.+https://raw\.GIT_HUBusercontent\.com" $file >> $FileSurge
done
echo -e "  ]\n}" >> $FileQx
